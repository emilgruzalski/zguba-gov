...
async def get_stats(db: AsyncSession = Depends(get_db)):
    ...
    available_result = await db.execute(
        select(func.count(FoundItem.id)).where(FoundItem.item_status == "available")
    )
    available_items = available_result.scalar()
    
    claimed_result = await db.execute(
        select(func.count(FoundItem.id)).where(FoundItem.item_status == "claimed")
    )
    ...
    top_municipalities = [
        ...
        for row in municipalities_result.all()
    ]
