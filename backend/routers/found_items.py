...
from schemas import FoundItemCreate, FoundItemResponse, FoundItemUpdate
...
async def get_found_items(
    ...
    category: str | None = Query(None, description="Item category"),
    municipality: str | None = Query(None, description="Municipality name"),
    status: str | None = Query(None, description="Status: available, claimed, expired"),
...
async def create_found_item(item_data: FoundItemCreate, db: AsyncSession = Depends(get_db)):
...
async def update_found_item(item_id: str, item_data: FoundItemUpdate, db: AsyncSession = Depends(get_db)):
    """Update a found item."""
    result = await db.execute(select(FoundItem).where(FoundItem.id == item_id))
...
async def delete_found_item(item_id: str, db: AsyncSession = Depends(get_db)):
    """Delete a found item."""
    result = await db.execute(select(FoundItem).where(FoundItem.id == item_id))
    db_item = result.scalar_one_or_none()
...
async def get_categories(db: AsyncSession = Depends(get_db)):
    ...
    result = await db.execute(select(FoundItem.item_category).distinct().order_by(FoundItem.item_category))
